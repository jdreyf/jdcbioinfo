#' Plot histograms of significance (p-value & FDR) columns
#'
#' Plot histograms of significance (p-value & FDR) and QQ (p-value) columns.
#'
#' @param p.suffix Suffix for p-value columns. P-value column names cannot be duplicated.
#' @param fdr.suffix Suffix for FDR columns. Set to \code{NA} if no FDR columns. FDR column names cannot be duplicated.
#' @param sep Separator for column names before \code{p} or \code{FDR}, passed to \code{\link{ezlimmaplot:::extract_prefix}}.
#' If not found, it is assumed to be \code{NA}.
#' @param pi0 Logical indicating if proportion of null hypotheses should be calculated per p-value histogram.
#' @param method Method for estimating pi0, passed to \code{\link[limma:propTrueNull]{limma::propTrueNull}}. Ignored if \code{!pi0}.
#' @inheritParams ezlimmaplot::ezheat
#' @inheritParams ezlimmaplot::ezvenn
#' @details Some p-value columns must be identifiable using \code{p.suffix}. If \code{!is.na(fdr.suffix)}, FDR
#' colnames must have same prefix.
#' @return List of ggplots.
#' @export

# assume each comparison has a p-value & q-value column unless fdr.suffix=NA
# could allow for no prefix, ie colnames(tab)=c("p", "FDR")
signif_hist_qq <- function(tab, p.suffix = "p", fdr.suffix = "FDR", sep = ".", pi0 = FALSE,
                           method = c("lfdr", "convest"), name = "signif_hist_qq") {
  stopifnot(nrow(tab) > 0, ncol(tab) > 0, !is.null(colnames(tab)))
  method <- match.arg(method)
  prefix.v <- ezlimmaplot:::extract_prefix(colnames(tab), suffix = p.suffix, sep = sep)

  if (any(duplicated(prefix.v))) stop("p-value column names are duplicated.")

  if (is.na(prefix.v[1])) {
    p.cols <- colnames(tab)[match(p.suffix, colnames(tab))]
  } else {
    p.cols <- colnames(tab)[match(paste0(prefix.v, sep, p.suffix), colnames(tab))]
  }

  if (!is.na(fdr.suffix)) {
    if (is.na(prefix.v[1])) {
      fdr.cols <- colnames(tab)[match(fdr.suffix, colnames(tab))]
    } else {
      fdr.cols <- colnames(tab)[match(paste0(prefix.v, sep, fdr.suffix), colnames(tab))]
    }
    if (any(is.na(fdr.cols))) stop("!is.na(fdr.suffix) but FDR columns not found.")
  }

  plist <- list()
  for (ind.tmp in seq_along(p.cols)) {
    prefix <- prefix.v[ind.tmp]
    p.col <- p.cols[ind.tmp]
    stopifnot(length(p.col) == 1)

    # ---------- p1 : histogram ----------
    pvals_hist <- tab[, p.col, drop = TRUE]
    pvals_hist <- pvals_hist[!is.na(pvals_hist)]

    # histogram bin count for y‑position of π₀ annotation
    hist_obj <- hist(pvals_hist, breaks = 20, plot = FALSE)
    max_count <- max(hist_obj$counts)

    p1 <- ggplot2::ggplot(tab, ggplot2::aes(x = !!rlang::sym(p.col))) +
      ggplot2::geom_histogram(bins = 20, fill = "gray", color = "white") +
      ggplot2::labs(x = "P-value", title = prefix) +
      ggplot2::geom_hline(yintercept = 0.05 * length(pvals_hist), linetype = "dashed") +
      ggplot2::annotate("text", x = 0.8, y = 0.05 * length(pvals_hist),
                        label = "Random", vjust = -1) +
      ggplot2::theme_minimal()

    # Add π₀ annotation if requested
    if (pi0) {
      if (!requireNamespace("limma", quietly = TRUE)) {
        stop("Package 'limma' needed to estimate pi0. Please install it.", call. = FALSE)
      }
      prop.null <- limma::propTrueNull(pvals_hist, method = method)

      # Decide position: top‑right if more p ≤ 0.5, else top‑left
      n_le_0.5 <- sum(pvals_hist <= 0.5)
      n_gt_0.5 <- sum(pvals_hist > 0.5)
      if (n_le_0.5 > n_gt_0.5) {
        x_pos <- 0.9   # top‑right
        hjust <- 1
      } else {
        x_pos <- 0.1   # top‑left
        hjust <- 0
      }
      y_pos <- max_count * 0.9   # near the top

      p1 <- p1 +
        ggplot2::annotate("text", x = x_pos, y = y_pos,
                          label = paste0("pi[0] == ", signif(prop.null, 3)),
                          parse = TRUE, hjust = hjust, vjust = 1, size = 4)
    }

    # ---------- p2 : Q–Q plot ----------
    tab <- tab %>%
      dplyr::mutate(expected = -log10((rank(!!rlang::sym(p.col), na.last = "keep") - 0.5) /
                                        sum(!is.na(!!rlang::sym(p.col)))),
                    observed = -log10(!!rlang::sym(p.col))) %>%
      dplyr::filter(!is.na(observed))

    # Lambda GC (1 df)
    pvals <- pvals_hist[pvals_hist > 0 & pvals_hist <= 1]   # already non‑NA
    chisq_obs <- stats::qchisq(1 - pvals, df = 1)
    lambda_gc <- median(chisq_obs, na.rm = TRUE) / stats::qchisq(0.5, df = 1)

    p2 <- ggplot2::ggplot(tab, ggplot2::aes(x = expected, y = observed)) +
      ggplot2::geom_point(size = 0.5, color = "gray") +
      ggplot2::geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
      ggplot2::labs(x = "Expected -log10(P-value)",
                    y = "Observed -log10(P-value)",
                    title = prefix) +
      ggplot2::theme_minimal() +
      # λGC always in top‑left
      ggplot2::annotate("text", x = -Inf, y = Inf,
                        label = paste0("lambda[GC] == ", signif(lambda_gc, 3)),
                        parse = TRUE, hjust = -0.1, vjust = 1.1, size = 4)

    # ---------- p3 : FDR histogram (optional) ----------
    if (!is.na(fdr.suffix)) {
      fdr.col <- fdr.cols[ind.tmp]
      stopifnot(length(fdr.col) == 1)

      p3 <- ggplot2::ggplot(tab, ggplot2::aes(x = !!rlang::sym(fdr.col))) +
        ggplot2::geom_histogram(bins = 20, fill = "gray", color = "white") +
        ggplot2::labs(x = "FDR", title = prefix) +
        ggplot2::theme_minimal()
    }

    plist[[prefix]] <- patchwork::wrap_plots(p1, p2,
                                             if (!is.na(fdr.suffix)) p3 else NULL,
                                             ncol = 2 + !is.na(fdr.suffix))
  }

  if (!is.na(name)) {
    grDevices::pdf(paste0(name, ".pdf"), width = 4 * length(plist[[1]]), height = 4)
    on.exit(grDevices::dev.off())

    for (prefix in names(plist)) {
      print(plist[[prefix]])
    }
  }
  return(invisible(plist))
}
