...
#'
#' Perform limma's pairwise moderated t-tests, find specifically up or down-regulated markers for each group vs all other groups. Uses maximum p-value, but
#' cannot divide by two since tests are dependent.
#'
...
limma_find_all_markers <- function(object, grp, direction=c("up", "down"), design=NULL,
  ...
  stopifnot(ncol(object)==length(grp), colnames(object)==names(grp), length(unique(grp))>1,
            direction %in% c("up", "down"), !is.null(design) || !is.null(grp), is.null(design) || nrow(design)==ncol(object),
            is.null(design) || all(rownames(design)==colnames(object)), !is.null(rownames(object)))
