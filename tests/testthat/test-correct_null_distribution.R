...
test_that("without prefix.v", {
  suppressWarnings(mtt <- correct_null_distribution(mtt))
  expect_equal(mtt[1, "Middle3vsFirst3.p"], 1.559628e-15)
})

test_that("with prefix.v", {
  suppressWarnings(mtt <- correct_null_distribution(mtt, prefix.v = names(contr.v)))
  expect_equal(mtt[1, "Middle3vsFirst3.p"], 1.559628e-15)
})
