...
test_that("without prefix.v", {
  expect_warning(mtt1 <- correct_null_distribution(mtt))
  expect_equal(mtt1[1, "Middle3vsFirst3.p"], 1.559628e-15)
})

test_that("with prefix.v", {
  expect_warning(mtt1 <- correct_null_distribution(mtt, prefix.v = names(contr.v)))
  expect_equal(mtt1[1, "Middle3vsFirst3.p"], 1.559628e-15)
})
...
test_that("Direction input", {
  ...
  expect_warning(mtt_d <- correct_null_distribution(mtt_d, direction.suffix="Direction"))
